...
func TestTxMarshaling(t *testing.T) {
	fr := newFramework(t)
	defer fr.Close()

	clt := fr.NewSDKClient()

	decodeTxAbi := artifacts.SuaveAbi.Methods["decodeTransaction"]
	decodeTxAddr := artifacts.SuaveMethods["decodeTransaction"]
	// encodeTxAbi := artifacts.SuaveAbi.Methods["encodeTransaction"]
	encodeTxAddr := artifacts.SuaveMethods["encodeTransaction"]
	...
	decodedTxReturn, err := clt.RPC().CallContract(context.TODO(), ethereum.CallMsg{
		To:   &decodeTxAddr,
		Data: calldata,
	}, nil)
	require.NoError(t, err)

	unpackedDecodedTxReturn, err := decodeTxAbi.Outputs.Unpack(decodedTxReturn)
	require.NoError(t, err)

	recoveredTxArgs := unpackedDecodedTxReturn[0].(struct {
	...
	encodedTxReturn, err := clt.RPC().CallContract(context.TODO(), ethereum.CallMsg{
		To:   &encodeTxAddr,
		Data: decodedTxReturn,
	}, nil)
	require.NoError(t, err)

	recoveredTx := types.Transaction{}
	err = recoveredTx.UnmarshalBinary(encodedTxReturn)
	require.NoError(t, err)
	...
	require.Equal(t, uint64(100), recoveredTx.GasPrice().Uint64())
	require.Equal(t, uint64(10), recoveredTx.Value().Uint64())
