...
func applyTemplate(templateText string, input desc, out string) error {
	...
	funcMap := template.FuncMap{
		...
		"typ2": func(param interface{}) string {
			return encodeTypeToGolang(param.(string), false, false, false)
		},
		"typ3": func(param interface{}) string {
			return encodeTypeToGolang(param.(string), true, true, false)
		},
		"hextype": func(param interface{}) string {
			return encodeTypeToGolang(param.(string), true, true, true)
		},
		"isHex": func(param interface{}) bool {
			typ := param.(string)
			return typ == "bytes" || typ == "uint64" || typ == "uint256"
		},
		"lower": func(param interface{}) string {
			return strings.ToLower(param.(string)[0:1]) + param.(string)[1:]
		},
...
func encodeTypeToGolang(str string, insideTypes bool, slicePointers bool, hexEncode bool) string {
	typ, err := abi.NewType(str, "", nil)
	if err == nil {
		// basic type that has an easy match with Go
		if typ.T == abi.SliceTy {
			return "[]" + encodeTypeToGolang(typ.Elem.String(), insideTypes, slicePointers, hexEncode)
		}
		...
		if strings.HasPrefix(str, "uint") {
			if str == "uint64" && hexEncode {
				return "hexutil.Uint64"
			}
...
{{range .Fields}}
{{if isHex .Typ}}{{title .Name}} {{hextype .Typ}}{{else}}
{{end}}{{end}}
}
