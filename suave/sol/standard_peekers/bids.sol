...
contract MevShareBidContract is AnyBidContract {
	...
	function otherCallback(Suave.BidId bid) public payable {
		emit HintEvent(bid, abi.encode(1));
	}

	function other(Bundle memory bundle) public returns (bytes memory) {
		...
		return abi.encodeWithSelector(this.otherCallback.selector, bid.id);
	}

	function newMatch2(Bundle memory bundle, Suave.BidId shareBidId) public returns (bytes memory) {
		address[] memory allowedList = new address[](1);
        allowedList[0] = address(this);
