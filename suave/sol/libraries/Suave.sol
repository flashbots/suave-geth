...
library Suave {
    ...
    address public constant DERIVE_PUBLIC_ADDRESS = 0x0000000000000000000000000000000053200004;
    ...
    /// @notice Derives a public address from a private key in ECDA secp256k1 format
    /// @param privateKey Hex encoded string of the ECDSA private key. Exactly as generated by privateKeyGen.
    /// @param crypto Type of the private key
    /// @return publicAddress Public address derived from the private key
    function derivePublicAddress(string memory privateKey, CryptoSignature crypto) internal returns (address) {
        (bool success, bytes memory data) = DERIVE_PUBLIC_ADDRESS.call(abi.encode(privateKey, crypto));
        if (!success) {
            revert PeekerReverted(DERIVE_PUBLIC_ADDRESS, data);
        }
