...
type BuildPayloadArgs types.BuildBlockArgs
...
func (w *worker) buildPayload(args *BuildPayloadArgs) (*Payload, error) {
	...
	empty, _, err := w.getSealingBlock(args.Parent, args.Timestamp, args.FeeRecipient, args.Random, args.Withdrawals, false, args.Transactions)
	if err != nil {
		return nil, err
	}
	...
	go func() {
		...
		for {
			...
			case <-timer.C:
				start := time.Now()
				block, fees, err := w.getSealingBlock(args.Parent, args.Timestamp, args.FeeRecipient, args.Random, args.Withdrawals, false, args.Transactions)
				if err == nil {
