...
type SuaveRuntime interface {
	...
	submitEthBundleToBuilder(builderUrl string, bundle []byte) ([]byte, error)
}
...
func (b *SuaveRuntimeAdapter) submitEthBundleToBuilder(input []byte) (res []byte, err error) {
	...
	bundle = unpacked[1].([]byte)

	var (
		output1 []byte
	)

	if output1, err = b.impl.submitEthBundleToBuilder(builderUrl, bundle); err != nil {
		return
	}
