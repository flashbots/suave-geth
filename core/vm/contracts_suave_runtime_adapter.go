...
type SuaveRuntime interface {
	...
	secp256k1Decrypt(ciphertext []byte, privateKey common.Hash) ([]byte, error)
	secp256k1Encrypt(message []byte, publicKey []byte) ([]byte, error)
...
func (b *SuaveRuntimeAdapter) run(addr common.Address, input []byte) ([]byte, error) {
	...
	case secp256k1DecryptAddr:
		return b.secp256k1Decrypt(input)

	case secp256k1EncryptAddr:
		return b.secp256k1Encrypt(input)
...
func (b *SuaveRuntimeAdapter) secp256k1Decrypt(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["secp256k1Decrypt"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	ciphertext = unpacked[0].([]byte)
	privateKey = unpacked[1].(common.Hash)

	var (
		message []byte
	)

	if message, err = b.impl.secp256k1Decrypt(ciphertext, privateKey); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["secp256k1Decrypt"].Outputs.Pack(message)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
...
func (b *SuaveRuntimeAdapter) secp256k1Encrypt(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["secp256k1Encrypt"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	message = unpacked[0].([]byte)
	publicKey = unpacked[1].([]byte)

	var (
		ciphertext []byte
	)

	if ciphertext, err = b.impl.secp256k1Encrypt(message, publicKey); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["secp256k1Encrypt"].Outputs.Pack(ciphertext)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
