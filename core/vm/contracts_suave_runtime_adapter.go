...
type SuaveRuntime interface {
	...
	getCallData(txn []byte) ([]byte, error)
...
func (b *SuaveRuntimeAdapter) ensureTxnValid(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["ensureTxnValid"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if err = b.impl.ensureTxnValid(txn); err != nil {
		return
	}
...
func (b *SuaveRuntimeAdapter) getCallData(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["getCallData"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if calldata, err = b.impl.getCallData(txn); err != nil {
		return
	}
