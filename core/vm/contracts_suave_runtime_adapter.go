...
var (
	...
	getBlockNumberAddr           = common.HexToAddress("0x0000000000000000000000000000000543200001")
...
var addrList = []common.Address{
	buildEthBlockAddr, confidentialInputsAddr, confidentialRetrieveAddr, confidentialStoreAddr, ethcallAddr, extractHintAddr, fetchBidsAddr, fillMevShareBundleAddr, getBlockNumberAddr, newBidAddr, signEthTransactionAddr, simulateBundleAddr, submitBundleJsonRPCAddr, submitEthBlockBidToRelayAddr,
}
...
func (b *SuaveRuntimeAdapter) run(addr common.Address, input []byte) ([]byte, error) {
	...
	case getBlockNumberAddr:
		return b.getBlockNumber(input)
...
func (b *SuaveRuntimeAdapter) getBlockNumber(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["getBlockNumber"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if number, err = b.impl.getBlockNumber(); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["getBlockNumber"].Outputs.Pack(number)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
