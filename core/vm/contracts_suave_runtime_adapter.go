...
type SuaveRuntime interface {
	...
	dagRetrieve(key common.Hash) ([]byte, error)
	dagStore(key common.Hash, value []byte) error
...
func (b *SuaveRuntimeAdapter) run(addr common.Address, input []byte) ([]byte, error) {
	...
	case dagRetrieveAddr:
		return b.dagRetrieve(input)

	case dagStoreAddr:
		return b.dagStore(input)
...
func (b *SuaveRuntimeAdapter) dagRetrieve(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["dagRetrieve"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if value, err = b.impl.dagRetrieve(key); err != nil {
		return
	}
...
func (b *SuaveRuntimeAdapter) dagStore(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["dagStore"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	key = unpacked[0].(common.Hash)
	value = unpacked[1].([]byte)

	var ()

	if err = b.impl.dagStore(key, value); err != nil {
		return
	}
