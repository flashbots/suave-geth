...
type SuaveRuntime interface {
	...
	newDataRecord(decryptionCondition uint64, allowedPeekers []common.Address, allowedStores []common.Address, namespace string) (types.DataRecord, error)
...
func (b *SuaveRuntimeAdapter) newDataRecord(input []byte) (res []byte, err error) {
	...
	namespace = unpacked[3].(string)

	var (
		dataRecord types.DataRecord
	)

	if dataRecord, err = b.impl.newDataRecord(decryptionCondition, allowedPeekers, allowedStores, namespace); err != nil {
		return
	}
