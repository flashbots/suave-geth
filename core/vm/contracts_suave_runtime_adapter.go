...
// Hash: 935373e7d0c1cde29e8ab8e44beae995ab3844f99edfb7ccae4ab94939c186ef
package vm
...
type SuaveRuntime interface {
	...
	getb(cid []byte, timeout uint64) ([]byte, error)
...
var (
	...
	getbAddr                  = common.HexToAddress("0x0000000000000000000000000000000042019001")
	...
	putbAddr                  = common.HexToAddress("0x0000000000000000000000000000000042019002")
...
func (b *SuaveRuntimeAdapter) getb(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["getb"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	cid = unpacked[0].([]byte)
	timeout = unpacked[1].(uint64)

	var (
		data []byte
	)

	if data, err = b.impl.getb(cid, timeout); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["getb"].Outputs.Pack(data)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
...
func (b *SuaveRuntimeAdapter) putb(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["putb"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if cid, err = b.impl.putb(data); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["putb"].Outputs.Pack(cid)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
