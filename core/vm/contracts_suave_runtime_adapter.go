...
type SuaveRuntime interface {
	...
	submitBundleToBuilder(builderUrl string, bid types.BidId) ([]byte, error)
...
func (b *SuaveRuntimeAdapter) submitBundleToBuilder(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["submitBundleToBuilder"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
