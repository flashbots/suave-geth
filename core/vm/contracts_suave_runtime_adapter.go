...
type SuaveRuntime interface {
	...
	mossAddTransaction(txn []byte) ([]types.SimulatedLog, bool, string, error)
	mossSendBundle(bundle []byte) error
...
func (b *SuaveRuntimeAdapter) run(addr common.Address, input []byte) ([]byte, error) {
	...
	case mossAddTransactionAddr:
		return b.mossAddTransaction(input)

	case mossSendBundleAddr:
		return b.mossSendBundle(input)
...
func (b *SuaveRuntimeAdapter) mossAddTransaction(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["mossAddTransaction"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if logs, success, error, err = b.impl.mossAddTransaction(txn); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["mossAddTransaction"].Outputs.Pack(logs, success, error)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
...
func (b *SuaveRuntimeAdapter) mossSendBundle(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["mossSendBundle"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if err = b.impl.mossSendBundle(bundle); err != nil {
		return
	}
