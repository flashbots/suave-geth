...
type SuaveRuntime interface {
	...
	encodeRLPTxn(txn types.STransaction) ([]byte, error)
	...
	sendBundle(url string, bundle types.Bundle) error
	sendMevShareBundle(url string, bundle types.MevShareBundle) error
	signEthTransaction(txn []byte, chainId string, signingKey string) ([]byte, error)
	simulateBundle(bundleData []byte) (uint64, error)
	simulateTransactions(txn []types.STransaction) (uint64, error)
...
var addrList = []common.Address{
	buildEthBlockAddr, confidentialInputsAddr, confidentialRetrieveAddr, confidentialStoreAddr, encodeRLPTxnAddr, ethcallAddr, extractHintAddr, fetchBidsAddr, fillMevShareBundleAddr, newBidAddr, sendBundleAddr, sendMevShareBundleAddr, signEthTransactionAddr, simulateBundleAddr, simulateTransactionsAddr, submitBundleJsonRPCAddr, submitEthBlockBidToRelayAddr,
}
...
func (b *SuaveRuntimeAdapter) run(addr common.Address, input []byte) ([]byte, error) {
	...
	case sendBundleAddr:
		return b.sendBundle(input)

	case sendMevShareBundleAddr:
		return b.sendMevShareBundle(input)
	...
	case simulateTransactionsAddr:
		return b.simulateTransactions(input)
...
func (b *SuaveRuntimeAdapter) encodeRLPTxn(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["encodeRLPTxn"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if rlpTxn, err = b.impl.encodeRLPTxn(txn); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["encodeRLPTxn"].Outputs.Pack(rlpTxn)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
...
func (b *SuaveRuntimeAdapter) sendBundle(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["sendBundle"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if err = mapstructure.Decode(unpacked[1], &bundle); err != nil {
		err = errFailedToDecodeField
		return
	}
	...
	if err = b.impl.sendBundle(url, bundle); err != nil {
		return
	}
...
func (b *SuaveRuntimeAdapter) sendMevShareBundle(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["sendMevShareBundle"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if err = b.impl.sendMevShareBundle(url, bundle); err != nil {
		return
	}
...
func (b *SuaveRuntimeAdapter) simulateTransactions(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["simulateTransactions"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if profit, err = b.impl.simulateTransactions(txn); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["simulateTransactions"].Outputs.Pack(profit)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
