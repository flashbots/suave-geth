...
type SuaveRuntime interface {
	...
	submitPrompt(prompt string) (common.Address, error)
}
...
func (b *SuaveRuntimeAdapter) run(addr common.Address, input []byte) ([]byte, error) {
	...
	case submitPromptAddr:
		return b.submitPrompt(input)
...
func (b *SuaveRuntimeAdapter) submitPrompt(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["submitPrompt"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	prompt = unpacked[0].(string)

	var (
		maybeHash common.Address
	)

	if maybeHash, err = b.impl.submitPrompt(prompt); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["submitPrompt"].Outputs.Pack(maybeHash)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
