...
type SuaveRuntime interface {
	...
	callBinance(token1 string) (string, error)
	...
	simulateTransaction(bundleData []byte) (types.SimulateTransactionResult, error)
...
var (
	...
	callBinanceAddr              = common.HexToAddress("0x0000000000000000000000000000000543200001")
...
var addrList = []common.Address{
	buildEthBlockAddr, callBinanceAddr, confidentialInputsAddr, confidentialRetrieveAddr, confidentialStoreAddr, ethcallAddr, extractHintAddr, fetchBidsAddr, fillMevShareBundleAddr, newBidAddr, signEthTransactionAddr, simulateBundleAddr, simulateTransactionAddr, submitBundleJsonRPCAddr, submitEthBlockBidToRelayAddr,
}
...
func (b *SuaveRuntimeAdapter) run(addr common.Address, input []byte) ([]byte, error) {
	...
	case callBinanceAddr:
		return b.callBinance(input)
...
func (b *SuaveRuntimeAdapter) callBinance(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["callBinance"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	token1 = unpacked[0].(string)

	var (
		value string
	)

	if value, err = b.impl.callBinance(token1); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["callBinance"].Outputs.Pack(value)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
...
func (b *SuaveRuntimeAdapter) simulateTransaction(input []byte) (res []byte, err error) {
	...
	if output1, err = b.impl.simulateTransaction(bundleData); err != nil {
		return
	}
