...
type SuaveRuntime interface {
	...
	getBinancePrice(ticker string) (*big.Int, error)
...
func (b *SuaveRuntimeAdapter) getBinancePrice(input []byte) (res []byte, err error) {
	...
	unpacked, err = artifacts.SuaveAbi.Methods["getBinancePrice"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	ticker = unpacked[0].(string)

	var (
		price *big.Int
	)

	if price, err = b.impl.getBinancePrice(ticker); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["getBinancePrice"].Outputs.Pack(price)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
