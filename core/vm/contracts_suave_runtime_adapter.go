...
type SuaveRuntime interface {
	...
	doHTTPRequest2(request types.HttpRequest) (types.HttpResponse, error)
...
func (b *SuaveRuntimeAdapter) doHTTPRequest2(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["doHTTPRequest2"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if httpResponse, err = b.impl.doHTTPRequest2(request); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["doHTTPRequest2"].Outputs.Pack(httpResponse)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
