...
type SuaveRuntime interface {
	...
	simulateTransaction(bundleData []types.SimulatedTransaction) (types.SimulateTransactionResult, error)
...
var addrList = []common.Address{
	buildEthBlockAddr, confidentialInputsAddr, confidentialRetrieveAddr, confidentialStoreAddr, ethcallAddr, extractHintAddr, fetchBidsAddr, fillMevShareBundleAddr, newBidAddr, signEthTransactionAddr, simulateBundleAddr, simulateTransactionAddr, submitBundleJsonRPCAddr, submitEthBlockBidToRelayAddr,
}
...
func (b *SuaveRuntimeAdapter) simulateTransaction(input []byte) (res []byte, err error) {
	...
	var (
		bundleData []types.SimulatedTransaction
	)

	if err = mapstructure.Decode(unpacked[0], &bundleData); err != nil {
		err = errFailedToDecodeField
		return
	}
