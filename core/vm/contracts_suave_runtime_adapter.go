...
type SuaveRuntime interface {
	...
	submitEthBundleToBuilder(builderUrl string, bid types.BidId) ([]byte, error)
}
...
func (b *SuaveRuntimeAdapter) submitEthBundleToBuilder(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["submitEthBundleToBuilder"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	builderUrl = unpacked[0].(string)

	if err = mapstructure.Decode(unpacked[1], &bid); err != nil {
		err = errFailedToDecodeField
		return
	}
	...
	if output1, err = b.impl.submitEthBundleToBuilder(builderUrl, bid); err != nil {
		res = []byte(err.Error())
		return
	}
