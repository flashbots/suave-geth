...
type SuaveRuntime interface {
	...
	simulateBundle(bundleData []byte) (uint64, bool, error)
...
func (b *SuaveRuntimeAdapter) simulateBundle(input []byte) (res []byte, err error) {
	...
	if output1, failed, err = b.impl.simulateBundle(bundleData); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["simulateBundle"].Outputs.Pack(output1, failed)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
