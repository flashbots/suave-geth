...
type SuaveRuntime interface {
	...
	derivePublicAddress(privateKey string, crypto types.CryptoSignature) (common.Address, error)
...
var (
	...
	derivePublicAddressAddr   = common.HexToAddress("0x0000000000000000000000000000000053200004")
...
func (b *SuaveRuntimeAdapter) run(addr common.Address, input []byte) ([]byte, error) {
	...
	case derivePublicAddressAddr:
		return b.derivePublicAddress(input)
...
func (b *SuaveRuntimeAdapter) derivePublicAddress(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["derivePublicAddress"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if publicAddress, err = b.impl.derivePublicAddress(privateKey, crypto); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["derivePublicAddress"].Outputs.Pack(publicAddress)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
