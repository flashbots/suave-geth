...
type BackendImpl interface {
	buildEthBlock(blockArgs types.BuildBlockArgs, bid [32]byte, namespace string) ([]byte, []byte, error)
	confidentialStoreRetrieve(bidId [32]byte, key string) ([]byte, error)
	confidentialStoreStore(bidId [32]byte, key string, data []byte) error
...
func (b *BackendStub) buildEthBlock(input []byte) ([]byte, error) {
	...
	if res0, res1, err = b.impl.buildEthBlock(blockArgs, unpacked[1].([32]byte), unpacked[2].(string)); err != nil {
		return nil, err
	}
...
func (b *BackendStub) confidentialStoreRetrieve(input []byte) ([]byte, error) {
	...
	if res0, err = b.impl.confidentialStoreRetrieve(unpacked[0].([32]byte), unpacked[1].(string)); err != nil {
		return nil, err
	}
...
func (b *BackendStub) confidentialStoreStore(input []byte) ([]byte, error) {
	...
	if err = b.impl.confidentialStoreStore(unpacked[0].([32]byte), unpacked[1].(string), unpacked[2].([]byte)); err != nil {
		return nil, err
	}
...
func (b *BackendStub) newBid(input []byte) ([]byte, error) {
	...
	if res0, err = b.impl.newBid(unpacked[0].(uint64), unpacked[1].([]common.Address), unpacked[2].([]common.Address), unpacked[3].(string)); err != nil {
		return nil, err
	}
